...
param.mean_amplification_thresh = 1000000000; % this gets rid of ROIs with bad F0 fits when computing delta F over F
...
for i_ex = param.analyze_these
    ...
    % exp_info.time is the time in seconds the ith frame was recorded
    % exp_info.epoch_val is the epoch index at the ith frame
    ...
    plotH = figure; hold on
    imagesc(roi_raw_intensity')
    %xticklabels( mat2str(exp_info.time / 60) )
    %XTick(exp_info.time / 60)
    xlabel('time (indices)')
    ylabel('ROI')
    title(['Fly ', num2str( param.fly_num ), ' Raw Intensity Over Time'])
    set(gca, 'fontsize', 20)
    ...
    if false
        ...
        if num_rois > 0
            ...
            % plot the dx and dy
            plotH = figure('Color',[1 1 1]); hold on
            subplot(2, 1, 1)
            % subplot of dx change
            plot(exp_info.time ./ 60, dx_filt, 'linewidth', 1) % seconds
            xlabel('time (minutes)')
            ylabel('dx (pixels)')
