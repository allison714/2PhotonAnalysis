%% --- I. Load experimental details and paths ---
clear; clc; close all;
...
param.corr_thresh = 0.000000000000002; % correlation threshold [0.2]
...
param.mean_amplification_thresh = 100; % this gets rid of ROIs with bad F0 fits when computing delta F over F
...
for i_ex = param.analyze_these
    ...
    if num_rois > 0
        ...
        % ------------------------------------------------------------------------------------
        filename = sprintf('%s_meanImgVsweepFly%d.png',savetime,param.fly_num);

        saveas( gcf,fullfile('C:\Users\Lab_User\Documents\Allison Cairns\2022-2023\Imaging\vsweep_02', filename),'png')
...
% % % 
% % %     for ep = [6:15]
% % %         ids = bwlabel( resp{1}.epoch_trace == ep );
% % %         xq = resp{1}.time( ids == 1 );
% % %         xq = xq - xq(1);
% % % 
% % %         vq = zeros(length(xq), size(resp{1}.dff,2), max(ids));
% % %         for id = 1 : max(ids)
...
% % %         for i_roi = [1:size(resp{1}.dff,2)]
% % %             this_mean = mean(vq(:,i_roi,:),3);
% % %             this_sem = std_error(:,i_roi);
% % %             plot( xq, this_mean , 'Color', colors(i_roi,:))
% % %             patch([xq, fliplr(xq)], [this_mean-this_sem; flipud(this_mean + this_sem)]', colors(i_roi,:),'LineStyle','none', 'FaceAlpha',.3)
% % %         end
% % % 
% % %         % plot(xq, vq)
% % % 
% % %         title(['Fly: ', num2str(param.fly_num) ,' | Epoch: ', num2str(ep)])
% % %         xlabel('t (s)')
% % %         ylabel('$\Delta$ F/F', 'Interpreter', "Latex")
% % % 
% % %         filename = sprintf('%s_avgVsweepFly%dEp%d.png',savetime,param.fly_num,ep);
% % %         saveas( gcf,fullfile('C:\Users\Lab_User\Documents\Allison Cairns\2022-2023\Imaging\vsweep_02', filename),'png')
% % %     end
    %%
    color = linspecer(7);
    ...
    for ep = [6:15]
        ...
        for i_roi = [1:size(resp{1}.dff,2)]
            ...
            filename = sprintf('%s_allEpVsweepFly%d.png',savetime,param.fly_num);
            saveas( gcf,fullfile('C:\Users\Lab_User\Documents\Allison Cairns\2022-2023\Imaging\vsweep_02', filename),'png')
        end
        ...
        xlabel('t (s)')
        ylabel('$\Delta$ F/F', 'Interpreter', "Latex")
        sgtitle(['Fly: ', num2str(param.fly_num)])
