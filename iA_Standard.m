%% Export Imaging Analysis Workspace - STANDARD THRESHOLDS
% A. Cairns || 08.08.2023
...
%% II. Data Analysis - change this so nothresh and standard work together *****
...
% a. ROI selection
param.corr_thresh = 0.9; % correlation threshold [0.2]
param.probe_correlation = true;
param.probe_correlation_type = 'probe indices'; % gets ROI corr. from the 1st and 2nd probes
param.mean_amplification_thresh = 1.2; % gets rid of ROIs with bad F0 fits when computing delta F over F
...
elseif length(promptAns{6,1}) == length('true')
    param.manual_roi = 'true';
    param.manual_roi_name = 'ManualROI';
elseif length(promptAns{6,1}) == length('standard')
    param.manual_roi = 'false';
    param.manual_roi_name = 'Standard';
end
param.group_method = 'none';
param.frac_interleave = 0.5; % fraction used for computing F0 for DF/F ** why does psy5 use 0.4?
%% III. Run Analysis
